...
public class Genetic_Algorithm_indiv
{
    ...
    {   //m_realdata.forEach((k,m)-> System.out.println(k));
        ...
        while(true) {
            ...
            if (gen==30)
            {
                System.out.println(m_uid+ " s_id 0 kmeans "+ m_sid +" finish "+  population.get(population.size()-1).getScore()+" "+
                        population.get(population.size()-1).chromo);
                break;
            }
    ...
    private static class chromosome implements Comparable<chromosome>
    {
        ...
        private double getRandomGene(int gene)
        {
            ...
            if(gene == 1)
                return ran.nextInt(chromo.get(0).intValue()-5)+ 5+Precision.round(ran.nextDouble(),2);

            if(gene == 2) {
                return ran.nextInt(chromo.get(1).intValue()-4) + 4 + Precision.round(ran.nextDouble(),2);}
            return ran.nextInt(7)+8+Precision.round(ran.nextDouble(),2);
        }
