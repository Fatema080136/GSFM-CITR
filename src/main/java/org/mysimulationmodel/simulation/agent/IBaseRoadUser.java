...
public class IBaseRoadUser extends IBaseAgent<IBaseRoadUser>
{
    ...
    private static double m_decelerationcartocar = 3;
    private static double m_decelerationpedtocar = 3;
    //newly added
    ...
    public int donotstop = 0;

    /**
     * route
     */
    public List<Vector2d> m_route = new ArrayList<>();
    private int m_pedclass;
    public String name;
            ...
            for ( int i = 0; i < m_env.getRoadUserinfo().size(); i++ )
            {
                if ( !m_env.getRoadUserinfo().get(i).equals(this) )
                {
                    l_repulsetoOthers = CVector.add( l_repulsetoOthers, CForce.repulseotherPed(this, m_env.getRoadUserinfo().get(i),
                            m_pedlamda, m_repulsefactorpedtoped, m_pedtopedsigma, m_pedtocarsigma, m_repulsefactorpedtocar, m_env ) );
                }
            }
    ...
    public IBaseRoadUser nextPosition() throws Exception
    {
       ...
       if(this.getname().startsWith("t") && m_env.m_playedgame == 0)
        {
            ArrayList<String> l_temp =m_realdata.get( new StringBuffer(new StringBuffer(String.valueOf(m_env.getCurrentCycle()))
                    .append(this.getname()).append(m_env.getScenario())).toString());
            if( l_temp != null )
        ...
        if (this.getType() == 1)
        {
            m_acceleration = (this.m_currentbehavior == 0 || (this.m_currentbehavior == 2)) ? 0.1*m_pixelpermeter:
                    (this.m_currentbehavior == 3 && m_speed <= 0.204)? 0.93*m_pixelpermeter: 0.0;//*m_pixelpermeter;
            //l_acefactor = 0.0;
            l_acefactor = (this.m_currentbehavior == 0) ? 0.1: 0.0;
        }
        else
        {
        ...
        {    //System.out.println("am I "+this.m_route.size());
            //add if intermediate point is passed without reaching it, delete it from route list
            if ( ( this.getType() == 1 && l_distance <= 1.5*m_pixelpermeter ) || l_distance <= m_pixelpermeter )//10
            {
    ...
    private IBaseRoadUser carStopping()
    {
        ...
        if( CVector.distance( this.getPosition(), this.getCurrentlyPlayingWith().get(0).getPosition()) <= m_cartopeddistance_2 ) //7
        {
            m_speed = this.m_velocity.length();
    ...
    private IBaseRoadUser decelerating()
    {
        double l_distance = CVector.distance( this.m_position, this.getCurrentlyPlayingWith()
                .get(this.getCurrentlyPlayingWith().size()-1).getPosition()) - m_cartopeddistance;//7
    ...
    public void setPedClass(int m_pedtype)
    {
        m_pedclass = m_pedtype;
    }

    public IBaseRoadUser updateParameter( double p_pedtopedsigma, double p_pedtocarsigma, double p_pedtocardistance,
                                          double p_pedlamda, double p_pedtopedforce, double p_pedtocarforce,
                                          double p_cartocardistance, double p_pedtocardevdistance, String p_uid )
    {   name = p_uid;
        m_acceleration = 0.30*m_pixelpermeter;
        ...
        m_cartocardistance = p_cartocardistance;
        m_pedtocardistance = p_pedtocardistance;
        m_pedtocaraccelarationdistance = p_pedtocardevdistance;
        return this;
    }
    public void updateParameter(double v, double v1, String p_uid)
    {
