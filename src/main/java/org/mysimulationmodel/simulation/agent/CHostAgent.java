...
public final class CHostAgent extends IBaseAgent<CHostAgent>
{
    ...
    final void perceive()
    {
        ...
        m_env.getCarinfo().stream()
                ...
                .forEach( i ->
                {
                    ...
                    // for individual calibration
                     /*if( !( i.getCarfollowingActive()== 1
                            && i.getCarFollowingCar().getCurrentlyPlayingWith().contains(m_ped) ) &&
                            CVector.distance( i.getPosition(), m_ped.getPosition() ) <= m_distance )
                    {
                        double l_angle2 = CForce.getViewAngle( l_desireddirection.x, l_desireddirection.y,
                                ( m_ped.getPosition().x - i.getPosition().x), ( m_ped.getPosition().y - i.getPosition().y ) );

                        if ( ( l_angle2 <= 8 || l_angle2 >= 352 ) && !i.getCurrentlyPlayingWith().contains(m_ped))
                        {   //System.out.println( i.getname() + " "+ j.getname()+" pedinfrontangle "+l_angle2 );
                            l_pedinfront.add(m_ped);
                        }
                        if( ( ( l_angle2 > 58 && l_angle2 <= 113 ) || ( l_angle2 >= 247 && l_angle2 < 302 ) )
                            && CForce.collisionCheckingfordeceleration( m_ped,i,m_collisionchekingfactor ) )
                        {   // System.out.println( i.getname() + " "+ j.getname()+" devangle "+l_angle2 );
                            i.getDeviateList().add(m_ped);
                        }
                        //90, 270
                        if ( ( l_angle2 <= 113 || l_angle2 >= 247 ) && CForce.collisionCheckingfordeceleration( m_ped,i,m_collisionchekingfactor ) )
                        {   //System.out.println( i.getname() + " "+ j.getname()+" gameangle "+l_angle2 );
                            l_neighbourlist.add( m_ped );
                        }
                    }
                    ...
                    else
                    {
                        ...
                        final IBaseRoadUser[] l_test={null}; l_test[0] = m_ped;


                        if (!l_pedinfront.isEmpty() )//if ( l_test[0] != null  && !l_pedinfront.isEmpty() && ( l_pedinfront.contains(l_test[0]) ) )
                        {
                    ...
                    */
                    ...
                    m_env.getPedestrianinfo().stream().filter( k -> !i.equals( k ) && !( i.getCarfollowingActive()== 1
                            ...
                            CVector.distance( i.getPosition(), k.getPosition() ) <= i.m_distance ).forEach( j ->
                    {
                        double l_angle2 = CForce.getViewAngle( l_desireddirection.x, l_desireddirection.y,
                    ...
                    if(!l_neighbourlist.isEmpty() || !l_pedinfront.isEmpty())
                    {
                        //if compitative users are not in ego user's FOV then delete them from getcurrentlyplayingwith() list,
                        // otherwise delete from l_neighborlist
                        for( int l= 0; l<i.getCurrentlyPlayingWith().size(); l++)
        ...
        // not required for individual calibration
    ...
    public void triggerAgents( )
    {
        l_viewedroadusers.forEach( ( IBaseRoadUser i, List<IBaseRoadUser> m ) ->
        {
            ...
            {   m_env.m_playedgame = 1;
                ...
                ArrayList<Integer> l_strategyset = new ArrayList<>();

                //l_strategyset.add(0,1);
                l_strategyset.add(1,m_strategy);

                //to all pedestrians currently on car i's FOV
                if ( !i.get_competitiveUserList().isEmpty() && !l_strategyset.isEmpty() )
                {
                    {
                        j.beliefbase().remove(CLiteral.from("force", CRawTerm.from(3.0)));
                        j.beliefbase().remove(CLiteral.from("force", CRawTerm.from(1.0)));
                        j.beliefbase().remove(CLiteral.from("force", CRawTerm.from(0.0)));
                        j.beliefbase().remove(CLiteral.from("force", CRawTerm.from(2.0)));

                        j.beliefbase().add(CLiteral.from("force", CRawTerm.from((double)l_strategyset.get(1))));
                        j.setBehavior(l_strategyset.get(1));
                        m_env.setStrategy(j.getname(), l_strategyset.get(1) );
                        if (l_strategyset.get(1) == 0)
                            CForce.helpingfunctionofaccelaration(i, j);// need to fix this function

                        j.trigger(CTrigger.from(
                                ITrigger.EType.ADDGOAL, CLiteral.from(
                                        l_strategy.get(l_strategyset.get(1)))));

                        i.getCurrentlyPlayingWith().add(m_ped);
                        m_ped.getCurrentlyPlayingWith().add(0,i);
                        l_closedlist.add(m_ped);
    ...
    private ArrayList<Integer> getstrategies( final IBaseRoadUser p_self, final IBaseRoadUser p_other, final int p_gametype, final int p_row, final int p_column )
    {
        ...
        double l_distance =  CVector.subtract( p_self.getPosition(), p_other.getPosition()).length() - 6.1*m_pixelpermeter;
        ...
        double[][][] l_matrix = CGame.payoffMatrixCalculationCartoPed( 6.3, 10.4, 0.3, 1.1,0.4,(p_self.getSpeed())/m_pixelpermeter, l_angle, p_other.getSpeed()/m_pixelpermeter,
