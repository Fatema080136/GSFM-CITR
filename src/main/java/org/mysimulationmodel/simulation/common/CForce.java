...
public class CForce
{
    ...
    public static Vector2d repulseotherPed( final IBaseRoadUser p_self, final IBaseRoadUser p_other, double p_pedlamda,
                                            double p_repulsefactorpedtoped, double p_pedtopedsigma, double p_pedtocarsigma, double p_pedtocarforce,
                                            CEnvironment m_env)
    {
        double l_temp;
        if( p_other.getType()==2 )
        {
            Vector2d l_desireddirection = CVector.direction( p_other.getGoalposition(), p_other.getPosition() );
            double check = CVector.direction( p_self.getGoalposition(), p_self.getPosition() ).dot(l_desireddirection);
            if(check >= 0.99 || check <= -0.99) p_other.donotstop = 1;
            double angle=CForce.getViewAngle( l_desireddirection.x, l_desireddirection.y,
                    ( p_self.getPosition().x - p_other.getPosition().x), ( p_self.getPosition().y - p_other.getPosition().y ) );
            //
            if(( ((360 -angle) < 2 || angle < 2) || (check >= 0.99 || check <= -0.99)) && CVector.distance(p_self.getPosition(), p_other.getPosition()) < 10*m_pixelpermeter )
            {   m_env.m_playedgame = 1;
                double c = (check <= -0.99)? 1:1.5;
                if(p_self.getCurrentBehavior() != 2 && p_other.playgame == 0)
                {
                    double l_angle = 90; Vector2d dir;
                    if(angle >= 348)
                    {
                        double a = c*3*m_pixelpermeter ;
                        Vector2d f = CVector.scale(a,CVector.normalize(CVector.subtract(p_other.getGoalposition(), p_other.getPosition())));
                        double px = f.x * Math.cos(l_angle) - f.y * Math.sin(l_angle);
                        double py = f.x * Math.sin(l_angle) + f.y * Math.cos(l_angle);
                        dir = CVector.add( p_self.getPosition(),new Vector2d(px,py));

                        if ( p_self.getRoute().size() == 0 ) {p_self.getRoute().add(0, p_self.getGoalposition() );
                            p_self.setGoalPedestrian(dir);
                        }
                    ...
                    if(angle <= 12)
                    {
                        double a = c*2.2*m_pixelpermeter ;//+ 12/angle;
                        Vector2d f = CVector.scale(a,CVector.normalize(CVector.subtract(p_other.getGoalposition(), p_other.getPosition())));
                        double px = f.x * Math.cos(l_angle*2) - f.y * Math.sin(l_angle*2);
                        double py = f.x * Math.sin(l_angle*2) + f.y * Math.cos(l_angle*2);
                        dir = CVector.add( p_self.getPosition(),new Vector2d(px,py));
                        if ( p_self.getRoute().size() == 0 ) {p_self.getRoute().add(0, p_self.getGoalposition() );
                            p_self.setGoalPedestrian(dir);
                        }
                ...
                l_temp = p_self.getM_radius()*2 + p_other.getLengthradius()*1.5 - CVector.distance(p_self.getPosition(), p_other.getPosition());
                return CVector.scale(p_pedtocarforce/4 * Math.exp(l_temp/ p_pedtocarsigma), CVector.normalize(
                        CVector.subtract(p_self.getPosition(), p_other.getPosition())));
            }
            l_temp = p_self.getM_radius()/3 + p_other.getLengthradius()/1.5f - CVector.distance(p_self.getPosition(), p_other.getPosition());
            return CVector.scale(p_pedtocarforce/4 * Math.exp(l_temp/ p_pedtocarsigma), CVector.normalize(
                    CVector.subtract(p_self.getPosition(), p_other.getPosition())));
        }
        else {
    ...
    public static void helpingfunctionofaccelaration( IBaseRoadUser p_self, IBaseRoadUser p_other )//10
    {
        ...
        double l_scalingfactor = 6*m_pixelpermeter;
